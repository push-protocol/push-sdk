...
export const createUserIfNecessary = async (options: AccountEnvOptionsType): Promise<IConnectedUser | undefined> => {
  const { user } = options || {};
...
export const getChats = async (options: GetChatsType): Promise<GetChatsResponseType> => {
  const {
    account,
    pgpPrivateKey,
    supportAddress,
...
export const checkIfIntent = ({ chat, account }: CheckIfIntentType): boolean => {
  if (account) {
    if (
      ...
      chat.combinedDID.toLowerCase().includes(walletToPCAIP10(account).toLowerCase())
    ) {
      if (chat.intent && chat.intent.toLowerCase().includes(walletToPCAIP10(account).toLowerCase())) return false;
      else return true;
    }
  }
...
export const checkIfUnread = (chatId: string, chat: IFeeds): boolean => {
  ...
  if (tempChat && tempChat?.msg && tempChat.msg.timestamp! < chat.msg.timestamp!) return true;
  return false;
};

export const getChatId = ({ msg, account }: { msg: IMessageIPFS; account: string }) => {
...
export const appendUniqueMessages = (parentList: any[], newlist: any[], infront: boolean) => {
  const seenCids: { [key: string]: boolean } = {};

  const appendedArray = infront ? [...newlist, ...parentList] : [...parentList, ...newlist];

  const filteredArray = appendedArray.filter((el) => {
    // Check if the current element's cid has not been seen before
    if (!(el.cid in seenCids)) {
      // Add the current element's cid to the seenCids object
      seenCids[el.cid] = true;
      // Return true to keep the element in the filtered list
      return true;
    }
    // Return false to filter out duplicate elements
    return false;
  });

  return filteredArray;
  return appendedArray.filter((el) => {
