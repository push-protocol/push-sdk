...
export const MemberAlreadyPresent = (member: any, groupMembers: any) => {
  const memberCheck = groupMembers?.find(
    (x: any) => x.wallet?.toLowerCase() == member.wallets?.toLowerCase()
  );
  if (memberCheck) {
    return true;
  }
...
export const addWalletValidation = (
  ...
  const checkIfMemberisAlreadyPresent = MemberAlreadyPresent(
    member,
    groupMembers
  );
  ...
  if (
    member?.wallets?.toLowerCase() === walletToPCAIP10(account)?.toLowerCase()
  ) {
