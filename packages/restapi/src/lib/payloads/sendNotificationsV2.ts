...
import {
  ...
  getSource,
  getUUID,
  getPayloadForAPIInputV2,
  getVerificationProofV2,
} from './helpers';
import {
  getAPIBaseUrls,
  getCAIPAddress,
...
function validateOptions(options: ISendNotificationInputOptions) {
  if (!options?.channel) {
    throw '[Push SDK] - Error - sendNotificationV2() - "channel" is mandatory!';
  }
  if (!isValidPushCAIP(options.channel)) {
    throw '[Push SDK] - Error - sendNotificationV2() - "channel" is invalid!';
  }
  if (options.senderType === 0 && options.signer === undefined) {
    throw '[Push SDK] - Error - sendNotificationV2() - "signer" is mandatory!';
  }
  if (options.senderType === 1 && options.pgpPrivateKey === undefined) {
    throw '[Push SDK] - Error - sendNotificationV2() - "pgpPrivateKey" is mandatory!';
  }
  ...
  if (
    ...
    if (!options.notification) {
      throw '[Push SDK] - Error - sendNotificationV2() - "notification" mandatory for Identity Type: Direct Payload, Minimal!';
    }
    if (!options.payload) {
      throw '[Push SDK] - Error - sendNotificationV2() - "payload" mandatory for Identity Type: Direct Payload, Minimal!';
    }
  }
  ...
  if (
    ...
    throw new Error(
      '[Push SDK] - Error - sendNotificationV2() - Either chatId or rules object is required to send a additional meta notification for video or spaces'
    );
  }
}
...
export async function sendNotificationV2(options: ISendNotificationInputOptions) {
  try {
    const {
      /* 
    ...
    const notificationPayload = getPayloadForAPIInputV2(options, _recipients);

    const verificationProof = await getVerificationProofV2({
      senderType,
      signer,
      chainId,
      identityType,
    ...
    console.error(
      '[Push SDK] - Error - sendNotificationV2() - ',
      JSON.stringify(err)
    );
    throw err;
  }
