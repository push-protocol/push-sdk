...
const SearchGroupTest = () => {
  ...
  const testSearchGroups = async () => {
    try {
      setLoading(true);

      const response = await PushAPI.chat.search({
        searchTerm,
        pageNumber,
        pageSize,
        env
  ...
  return (
    <div>
      <h2>Search Group Test Page</h2>

      <Loader show={isLoading} />

      <Section>
        <SectionItem>
          <SectionButton onClick={testSearchGroups}>Search Groups</SectionButton>
        </SectionItem>
        <SectionItem>
...
export default SearchGroupTest;
