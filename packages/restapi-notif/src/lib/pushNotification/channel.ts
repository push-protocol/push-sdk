...
export class Channel extends PushNotificationBaseClass {
   ...
   */
  async info(channel?: string, options?: ChannelOptions) {
    try {
   ...
   */
  async search(query: string, options?: ChannelSearchOptions) {
    try {
      const {
   ...
   * @returns
   */
  async send(recipients: string[], options: NotificationOptions) {
    try {
  ...
  async update(options: CreateChannelOptions) {
    const {
      name,
      description,
      url,
   ...
   */
  async verify(channelToBeVerified: string) {
    try {
      this.checkSignerObjectExists();
  ...
  async setting(configuration: NotificationSettings) {
    try {
      this.checkSignerObjectExists();
      // check for PUSH balance
      const pushTokenContract = await this.createContractInstance(
